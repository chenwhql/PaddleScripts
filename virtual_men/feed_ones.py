...
def feed_ones(block, feed_target_names, batch_size=1):
    ...
    def fill_ones(var_name, batch_size):
        ...
        print var
        print var.dtype
        var = fluid.layers.lod_reset(x=var, target_lod=[0, 60])
        print var
        print var.dtype
        ...
        data = np.ones(np_shape, dtype=np_dtype)
        tensor = fluid.LoDTensor()
        tensor.set_recursive_sequence_lengths([[1]])
...
def feed_txt_shape(block, feed_target_names, input_shape, need_save=True):
    """
    """
    feed_dict = dict()
    def set_batch_size(shape, input_shape):
        return input_shape
    def fill_txt(var_name, shape, need_save):
        var = block.var(var_name)
        np_shape = set_batch_size(list(var.shape), shape)
        var_np = {
                core.VarDesc.VarType.BOOL: np.bool_,
        ...
        while data:
            ...
            data = fp_r.readline()
        fp_r.close()
        print('type: ', type(np_shape), np_shape)
        numpy_array = np.array(arry).reshape(np_shape).astype(np.float32) #np.random.random(np_shape).astype(np.float32)
        if need_save is True:
	        numpy_to_txt(numpy_array, 'feed_' + var_name + '.txt', True)
        return numpy_array
    for feed_target_name in feed_target_names:
        feed_dict[feed_target_name] = fill_txt(feed_target_name, input_shape, need_save)
    return feed_dict
...
def fluid_inference_test(model_path):
    ...
    with fluid.scope_guard(scope):
        ...
        if GLB_txt_path is None or GLB_txt_path == "":
            ...
            feed_list = feed_txt_shape(global_block, feed_target_names, GLB_input_shape)
