...
train_parameters = {   
    ...
    "label_dict": {}, 
    "base_dir": "data/data9692",
    "data_dir": "data/data9692/Images",  # 训练数据存储地址   
    ...
    "pretrained_dir": "work/GoogleNet_pretrained",    
    "mode": "train",   
    "num_epochs": 120,   
    "train_batch_size": 30,   
...
def init_train_parameters():
    ...
    :return:
    """
    train_file_list = os.path.join(train_parameters['base_dir'], train_parameters['train_file_list'])
    label_list = os.path.join(train_parameters['base_dir'], train_parameters['label_file'])
    index = 0
        ...
        for line in lines:
            if mode == 'train' or mode == 'eval':
                ...
                try:
                    ...
                    #logger.debug(f'读取图片出错:{img_path}')
...
def test_mapper(sample):
    ...
    img = paddle.dataset.image.load_image(img0)
    if img is None:
        print('出错图片：', img0)   
    #进行了简单的图像变换，这里对图像进行crop修剪操作，输出img的维度为(3, 200, 200)
    img = paddle.dataset.image.simple_transform(im=img,          #输入图片是HWC   
                                                resize_size=200, # 剪裁图片
                                                crop_size=200, 
                                                is_color=True,  #彩色图像
                                                is_train=False)
...
def train():
    ...
    file_list = os.path.join(train_parameters['base_dir'], train_parameters['train_file_list'])
    ...
    eval_list = os.path.join(train_parameters['base_dir'], train_parameters['eval_file_list'])
    test_reader = paddle.batch(custom_image_reader(eval_list, train_parameters['data_dir'], 'eval'),
                                batch_size=train_parameters['train_batch_size'],
                                drop_last=False)
    test_reader = paddle.reader.shuffle(test_reader, train_parameters['train_batch_size'])

    # tester_reader = test_r(test_list=eval_list)
    # test_reader = paddle.batch(paddle.reader.shuffle(
    #                             reader=tester_reader,buf_size=3000), 
    #                             batch_size=train_parameters['train_batch_size'])
    ...
    for pass_id in range(train_parameters["num_epochs"]):
        ...
        for step_id, data in enumerate(batch_reader()):
            ...
            if batch_id % 10 == 0:
                logger.info("第 {0} 轮, 第 {1} 批, loss {2}, acc1 {3}, 耗时 {4}".format(pass_id, batch_id, loss, acc1,
                                                                                            "%2.2f sec" % period))
             # 通用的保存策略，减小意外停止的损失
        ...
        for batch_id, data in enumerate(test_reader()):
            test_cost, test_acc, pred_ot  = exe.run(program=test_program, #执行训练程序
                                        feed=feeder.feed(data),               #喂入数据
                                        fetch_list=train_fetch_list)           #fetch 误差、准确率
        ...
        print('验证轮:%d, Cost:%0.5f, 准确度:%0.5f' % (pass_id, test_cost, test_acc))

        # 简单的提前停止策略，认为连续达到某个准确率就可以停止了
        if test_acc >= good_acc1:
            successive_count += 1
            logger.info("current test_acc {0} meets good {1}, successive count {2}".format(acc1, good_acc1, successive_count))
