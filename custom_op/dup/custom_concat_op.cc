...
std::vector<int64_t> ComputeOutShape(std::vector<std::vector<int64_t>> in_shapes, int64_t axis) {
  size_t n = in_shapes.size();
  auto out_shape = in_shapes[0];
  size_t zero_dim_size = out_shape.size();
  for (size_t i = 1; i < n; ++i) {
    PD_CHECK(in_shapes[i].size() == out_shape.size(), "Input dimension must be same.");
    for (size_t j = 0; j < zero_dim_size; ++j) {
      if (j == axis) {
        out_shape[axis] += in_shapes[i][j];
      } else {
        PD_CHECK(in_shapes[0][j] == in_shapes[i][j], "The ", j, "-th dimension of input must be same.");
      }
    }
  }
  return out_shape;
...
std::vector<paddle::Tensor> ConcatForwardDynamicAxis(const std::vector<paddle::Tensor>& inputs, const paddle::Tensor& axis_t) {
  // check inputs
  ...
  // calc
  PD_DISPATCH_FLOATING_AND_INTEGRAL_TYPES(inputs[0].type(), "ConcatCpuKernel",
    ([&] {
...
std::vector<paddle::Tensor> ConcatBackwardDynamicAxis(const std::vector<paddle::Tensor>& inputs,
  ...
  // calc
  PD_DISPATCH_FLOATING_AND_INTEGRAL_TYPES(grad_out.type(), "SplitCpuKernel",
    ([&] {
...
std::vector<std::vector<int64_t>> ConcatInferShapeDynamicAxis(
    std::vector<std::vector<int64_t>> input_shapes, std::vector<int64_t> axis_shape) {
