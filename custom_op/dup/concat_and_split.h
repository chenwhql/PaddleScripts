...
std::vector<int64_t> GetCols(const std::vector<paddle::Tensor>& ins, int64_t rows, int64_t* cols) {
...
void ConcatCpuKernel(const std::vector<paddle::Tensor>& ins, paddle::Tensor* out, int64_t axis) {
  size_t num = ins.size();
  ...
  for (size_t i = 0; i < num; ++i) {
    ...
    for (int j = 0; j < out_rows; ++j) {
      std::memcpy(
        out_data + j * out_cols + col_idx, in_data + j * col_len, sizeof(data_t) * col_len);
    }
    col_idx += col_len;
  }
}
...
template <typename data_t>
void SplitCpuKernel(
    const paddle::Tensor& in, const std::vector<paddle::Tensor>& ref_ins,
    std::vector<paddle::Tensor>* outs, int64_t axis) {
