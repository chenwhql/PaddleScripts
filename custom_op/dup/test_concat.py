...
custom_ops = load(
    ...
    extra_include_paths=test_include,
    verbose=True)


def concat_dynamic(func, device, dtype, np_inputs, axis_v):
    paddle.set_device(device)
    inputs = [paddle.to_tensor(x, dtype=dtype, place=device, stop_gradient=False) for x in np_inputs]
...
class TestCustomConcatDynamicAxisJit(unittest.TestCase):
    def setUp(self):
        ...
        self.devices = ['cpu']
        self.np_inputs = [np.array([[1, 2, 3], [4, 5, 6]]),
        ...
        for device in self.devices:
            for dtype in self.dtypes:
                for axis in self.axises:
                    out, grad_inputs = concat_dynamic(custom_ops.custom_concat, device, dtype, self.np_inputs, axis)
                    pd_out, pd_grad_inputs = concat_dynamic(paddle.concat, device, dtype, self.np_inputs, axis)
    ...
    def test_static(self):
        for device in self.devices:
            for dtype in self.dtypes:
                for axis in self.axises:
                    out, x1_grad, x2_grad = concat_static(custom_ops.custom_concat, device, dtype, self.np_inputs, axis)
                    pd_out, pd_x1_grad, pd_x2_grad = concat_static(paddle.concat, device, dtype, self.np_inputs, axis)
                    ...
                    self.assertTrue(
                        ...
                        "custom op x1_grad: {},\n paddle api x1_grad: {}".format(
                            x1_grad, pd_x1_grad))
                    self.assertTrue(
                        np.array_equal(x2_grad, pd_x2_grad),
                        "custom op x2_grad: {},\n paddle api x2_grad: {}".format(
                            x2_grad, pd_x2_grad))
