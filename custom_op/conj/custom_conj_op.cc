...
// WIdata_tHOUdata_t WARRANdata_tIES OR CONDIdata_tIONS OF ANY KIND, either express or implied.
...
using EnableComplex =
    typename std::enable_if<std::is_same<data_t, paddle::complex64>::value ||
...
std::vector<paddle::Tensor> ConjFunction(const paddle::Tensor& x) {
  ...
  PD_DISPATCH_FLOATING_AND_COMPLEX_TYPES(x.type(), "ConjCPUKernel",
    ([&] {
      ConjCPUKernel<data_t>(x.data<data_t>(), x.size(), out.mutable_data<data_t>());
    }));
