...
def conj_static_inference(np_data, np_label):
    paddle.enable_static()
    paddle.set_device("cpu")
    with static.scope_guard(static.Scope()):
        with static.program_guard(static.Program()):
            ...
            hidden = custom_ops.custom_conj(hidden)
...
class TestCustomConjJit(unittest.TestCase):
    ...
    def run_dynamic(self, dtype, np_input):
        out, x_grad = conj_dynamic(custom_ops.custom_conj,
                                              dtype, np_input)
        pd_out, pd_x_grad = conj_dynamic(
            paddle.conj, dtype, np_input)
