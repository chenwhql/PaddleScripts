...
class WrapEncoderLayer(Layer):
    def __init__(self, name_cope, src_vocab_size, max_length, n_layer, n_head,
                 ...
                 postprocess_cmd, weight_sharing, is_sparse=False):
        """
...
class TransFormer(Layer):
    def __init__(self,
        ...
        self._wrap_encoder_layer = WrapEncoderLayer(
            ...
            weight_sharing, is_sparse=is_sparse)
...
for is_sparse in [False, True]:
    ...
    print("- static graph avg cost: %.6f" % static_avg_cost_value[0])
    print("- dygrah avg cost: %.6f" % dy_avg_cost_value[0])
    print("- static graph sum cost: %.6f" % static_sum_cost_value[0])
    print("- dygrah sum cost: %.6f" % dy_sum_cost_value[0])
    if not np.array_equal(static_predict_value, dy_predict_value):
        print(static_predict_value)
        print(dy_predict_value)
    if not np.array_equal(static_token_num_value, dy_token_num_value):
        print(static_token_num_value)
        print(dy_token_num_value)   
